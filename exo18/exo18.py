...
def compress (arr, dict):
    for i in range(len(arr)):
        if arr[i] in dict:
            arr[i] = dict[arr[i]]
    return arr

def decompress (arr,dict):
...
def generateDico (arr):
    return {key : countOccurences(key,arr) for key in arr}

def generateRefDico (dict, occu, length) :
    refDico = {}
    for key, value in dict.items():
        if int(dict[key]) > occu and len(key)>length:
            refDico[key] = value
    return refDico
...
dico = generateRefDico(generateDico(parseStr(txt)), 2, 3)
txt_compressed = unparseStr(compress(parseStr(txt), dico))
